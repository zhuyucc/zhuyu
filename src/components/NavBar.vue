...
defineProps({ isDark: Boolean, themeMode: String })
    ...
    <div class="w-[90%] max-w-6xl mx-auto h-16 flex items-center justify-between px-4 sm:px-[30px]">
      ...
      <nav class="flex gap-8 text-sm font-bold">
        ...
        <button
          ...
          :title="themeMode === 'system' ? '跟随系统' : themeMode === 'dark' ? '切换白天模式' : '切换夜间模式'"
        >
          <svg v-if="themeMode === 'system'" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
          <svg v-else-if="isDark" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
